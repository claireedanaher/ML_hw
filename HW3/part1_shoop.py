...
def compute_a(z):
    ...
    # when z is very large, a goes towards 1
    # when z is very small, a goes towards 0
    if (z > 50):
        a = 1
    elif (z < -50):
        a = 0
...
# check y
    if (y == 1 and a != 0):
        ...
        L = -np.log(1 - a)
    # if a is such that we have negative logarithm of 0, then L is very small
    else:
        L = 1e10
...
def predict(Xtest, w, b):
    ...
    for i, x in enumerate(Xtest):
        ...
        # first need sigmoid function value
        z = compute_z(x, w, b)
        a = compute_a(z)
        
        # based on sigmoid value, predict Y[i] to be 0 or 1
        if (a >= 0.5):
            Y[i] = 1
        else:
            Y[i] = 0
