...
'''
    ...
    The main goal of this problem is to get familiar with a model-based classification method, 
    and how to train the model parameters on the training data.
    We will get familiar with gradient computation using the chain rule. 
    We will use cross entropy (log loss) as the loss function and 
    stochastic gradient descent to train the model parameters.
    ...
    Notations:
            ...
            ---------- partial gradients ----------------------
            dL_da: the partial gradient of the loss function L w.r.t. the activation a, 
                    a float scalar value. It represents (d_L / d_a)
            da_dz: the partial gradient of the activation a w.r.t. the logit z, 
                    a float scalar value. It represents (d_a / d_z)
            dz_dw: the partial gradients of the logit z w.r.t. the weights w, 
                    a numpy float matrix of shape (p by 1). It represents (d_z / d_w)
            ...
            dL_dw: the partial gradient of the loss function L w.r.t. 
                    the weight vector w, a numpy float matrix of shape (p by 1). 
...
def forward(x,y,w,b):
    '''
       Forward pass: given an instance in the training data, compute the logit z, activation a and 
               cross entropy L on the instance. 
        Input:
            x: the feature vector of a training instance, a float numpy matrix of shape p by 1. 
...
def compute_dL_da(a, y):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    # depending on y, use specific derivative function
    if (y == 1 and a != 0.):
        dL_da = -1/a
    elif (y == 0 and a != 1.):
        dL_da = 1/(1 - a)
    # cases where Divide-by-zero can occur 
    elif (y == 1 and a == 0.):
        dL_da = -1e10
    else:
        dL_da = 1e10
