...
def compute_da1_dz1(a1):
    ...
    da1_dz1=np.multiply(a1,np.subtract(1,a1))
...
def compute_dL_dz1(dL_da1,da1_dz1):
    ...
    #########################################
    ## INSERT YOUR CODE HERE
    mat=dL_da1.dot(da1_dz1.T)
    dL_dz1=[]
    for i in range(0, len(mat)):
        dL_dz1.append(mat[i,i])
    
    
    dL_dz1=np.matrix(dL_dz1)
    dL_dz1=dL_dz1.T
    #########################################
    return dL_dz1
...
def compute_gradients(dL_da2, da2_dz2, dz2_dW2, dz2_db2, dz2_da1, da1_dz1, dz1_dW1, dz1_db1):
    ...
    ## INSERT YOUR CODE HERE
    # the 2nd layer
    dL_dz_2=sr.compute_dL_dz(dL_da2,da2_dz2)
    dL_da1=compute_dL_da1(dL_dz_2,dz2_da1)
    dL_dz_1=compute_dL_dz1(dL_da1,da1_dz1)
    dL_dW2=sr.compute_dL_dW(dL_dz_2,dz2_dW2)
    dL_db2=sr.compute_dL_db(dL_dz_2,dz2_db2)
    dL_dW1=sr.compute_dL_dW(dL_dz_1,dz1_dW1)
    dL_db1=sr.compute_dL_db(dL_dz_1,dz1_db1)
    ...
    for _ in range(n_epoch):
        ...
        for x,y in zip(X,Y):
            ...
            # update the paramters using gradient descent
                        # update the paramters using gradient descent
            W1=np.subtract(W1, np.multiply(dL_dW1,alpha))
            b1=np.subtract(b1,np.multiply(dL_db1, alpha))
            W2=np.subtract(W2, np.multiply(dL_dW2,alpha))
            b2=np.subtract(b2,np.multiply(dL_db2, alpha))
