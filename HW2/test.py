...
def test_compute_Phi():
    ...
    Phi = compute_Phi(x,2) 
    print(type(Phi) == np.matrixlib.defmatrix.matrix)
    assert type(Phi) == np.matrixlib.defmatrix.matrix 
    print( np.allclose(Phi.T, [[1,1,1],[1,2,3]], atol = 1e-3) )
    assert np.allclose(Phi.T, [[1,1,1],[1,2,3]], atol = 1e-3) 
    

    Phi = compute_Phi(x,3) 
    print(np.allclose(Phi.T, [[1,1,1],[1,2,3],[1,4,9]], atol = 1e-3))
    assert np.allclose(Phi.T, [[1,1,1],[1,2,3],[1,4,9]], atol = 1e-3) 

    Phi = compute_Phi(x,4) 
    print(np.allclose(Phi.T, [[1,1,1],[1,2,3],[1,4,9],[1,8,27]], atol = 1e-3))
...
def test_compute_yhat():
    ...
    print(np.allclose(yhat, -0.7, atol = 1e-3) )
    assert np.allclose(yhat, -0.7, atol = 1e-3) 

    w = np.mat('-0.5; 0.6')
    yhat = compute_yhat(x.T,w)
    print(np.allclose(yhat, .7, atol = 1e-3) )
    ...
    print(np.allclose(yhat, [[0.52], [1.96], [1.2]], atol = 1e-3))
    ...
    x = np.mat(' 2., 0.8, 5.; 0.9, 6., 3.')
    z = compute_yhat(x,w)
    print(np.allclose(z, [[1.3],[3.33]], atol = 1e-3) )
    ...
    print(type(Y) == np.matrixlib.defmatrix.matrix)
    ...
    print(np.allclose(Y, Y_true, atol = 1e-2))
...
def test_compute_dL_dw():
    ...
    print(type(dL_dw) == np.matrixlib.defmatrix.matrix)
    assert type(dL_dw) == np.matrixlib.defmatrix.matrix
    print(dL_dw.shape == (2,1))
    assert dL_dw.shape == (2,1) 

    dL_dw_true =np.mat('0.10667; -0.004')
    print( np.allclose(dL_dw, dL_dw_true, atol = 1e-3))
...
def test_train():
    ...
    print(np.allclose(Xtrain.dot(w), Ytrain, atol = 1e-3) )
    ...
    print( np.allclose(Xtrain.dot(w), Ytrain, atol = 1e-2) )
