...
##CALCULATE ENTROPY OF SUBGROUPS
    ...
    def inference(t,x):
        ...
        xxx=0
        y=t.p
        while isleaf==False and isleaf!=None and xxx<10:
            xxx+=1
            #print('New')
            #print(i)
            entry=x[i]
    ...
    #--------------------------
    @staticmethod
    def load_dataset2(filename = 'credit.txt'):
        '''
            Load dataset 1 from the CSV file: 'data1.csv'. 
            The first row of the file is the header (including the names of the attributes)
            In the remaining rows, each row represents one data instance.
    ...
    def print_tree(t,X):
        ...
        for row in X:
            ...
            print(row_unique)
            check=[]
            for value in row_unique:
                check.append(value)
            X_unique.append(check)
        combinations=[]
        for row in X_unique:
            for val in row:
        ...
        tree_dict={}
        j=0
        while isleaf==False and isleaf!=None and xxx<5:
            xxx+=1
            attr_unique=np.unique(X[i])
            print(attr_unique)
            for attr in attr_unique:
                line_dict=tree_dict.get(j)
                if line_dict==None:
                    line_dict=(False,i,attr)
                print(line_dict)
                node=c.get(attr)
                if node != None:
                #print('next node')
